...
void Excersize_2_3(int solution, int answer)
{
	...
	if (solution == SHOW)
	{
		...
		printf("평균 속력의 공식은 : 평균속력 = (나중거리 - 처음거리 / 나중시간 - 처음시간) \n\n");
		printf("나중거리 = %.0lfkm\n처음거리 = %.0lf \n", distance, distance_0);
		printf("나중시간 = %.0lf 시 %.0lf 분\n처음시간 = %.0lf 시\n", time_h, time_m, time_0);
		printf("평균속력은 %.0lfkm - %.0lfkm / %.0lf시 %.0lf분 - %.0lf시\n", distance, distance_0, time_h, time_m, time_0);
		printf("km/h 를 m/s로 변환\n ");
		printf("%.0lfkm = %.0lfm x %.0lf의 세제곱\n2시간 37분 = %.0lf분 = %.0lf x %.0lf = %.0lf초\n", distance, distance, time_0, time_x, time_x, time_s, time_f);
	...
	if (answer == SHOW)
	{
		...
		printf("%.0lf x %.0lf 세제곱 / %.0lf = %.1lf\n", distance, time_0, time_f, velocity);
