...
void Excersize_2_3(int solution, int answer)
{
	...
	if (solution == SHOW)
	{
		...
		printf("왜냐하면 문제에서 열차의 평균속력을 묻고 있기에 평균속력을 구해야합니다.\n");
		printf("따라서 평균 속력의 공식은 : 평균속력 = (나중거리 - 처음거리 / 나중시간 - 처음시간)이다 \n\n");
		printf("나중거리는 %.0lfkm이고, 처음거리는 %.0lf이다. \n", distance, distance_0);
		printf("나중시간은 %.0lf 시 %.0lf 분이고, 처음시간은 %.0lf 시이다.\n", time_h, time_m, time_0);
		printf("평균속력은 %.0lfkm - %.0lfkm / %.0lf시 %.0lf분 - %.0lf시이며\n", distance, distance_0, time_h, time_m, time_0);
		printf("km/h 를 m/s로 변환해보면\n");
		printf("km는 m에 1000을 곱하면 되고,\n'h'에 3600을 곱하여 's'를 구하고 'm'에 60을 곱하여 's'를 구한다\n");
		printf("따라서 %.0lfkm = %.0lfm x 1000 = 408000이고\n2시간 37분 = %.0lf분 = %.0lf x %.0lf = %.0lf초이다\n", distance, distance, time_x, time_x, time_s, time_f);
	...
	if (answer == SHOW)
	{
		...
		printf("km/h에서 m/s로 변환한 값을 거리를 시간으로 나누면 된다\n");
